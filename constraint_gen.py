...
MovInstructions = Conj(
    Equal('inst', 'mov'),
    Disj(Conj(OpcodePair(0x88), Format_reg_rm),
         Conj(OpcodePair(0x8a), Format_rm_reg),
         Conj(OpcodePair(0xc6), Equal('modrm_opcode', 0), Format_imm_rm),
         Conj(OpcodePair(0xa0),
              MemoryAccessWithoutModRM,
              Equal('immediate_bytes', 4),
              GetAccArgRegname,
              Apply('args', Format, ['mem_prefix', 'acc_regname'],
                    '%sVALUE32, %s')),
         Conj(OpcodePair(0xa2),
              MemoryAccessWithoutModRM,
              Equal('immediate_bytes', 4),
              GetAccArgRegname,
              Apply('args', Format, ['acc_regname', 'mem_prefix'],
                    '%s, %sVALUE32')),
         Conj(ForRange('reg1', reg_count),
              GetArgRegname('reg1_name', 'reg1'),
              ...
              DataOperationWithoutModRM,
              DefaultImmediateSize,
              Apply('args', Format, ['immediate_desc', 'reg1_name'],
                    '%s, %s'))))
